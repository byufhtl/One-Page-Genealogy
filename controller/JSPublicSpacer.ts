...
class JSPublicSpacer implements  IStyler {
    ...
    private setBasedOnGeneration(parentBox: IBox, childBox: IBox, generation: number) {
        if(generation == 0) {
            ...
            if(numSpouses==0){
                childBox.setType('JSLargeDetRotPubBox');
                childBox.setX(bx);
                childBox.setHeight(BoxStyleFactory.getHeight('JSLargeDetRotPubBox'));
                childBox.setWidth(BoxStyleFactory.getWidth('JSLargeDetRotPubBox'));
