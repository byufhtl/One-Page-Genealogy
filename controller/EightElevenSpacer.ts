...
class EightElevenSpacer implements  IStyler {
    ...
    private setBasedOnGeneration(parentBox: IBox, childBox: IBox, generation: number) {
        ...
        if(generation == 0) {
            var bx = 0;
            childBox.setType('smallPictureBox2');
            childBox.setX(bx);
            childBox.setHeight(BoxStyleFactory.getHeight('smallPictureBox2'));
            childBox.setWidth(BoxStyleFactory.getWidth('smallPictureBox2'));
        }
        else if(generation == 1) {
            bx = parentBox.getX() + parentBox.getWidth()/2-20;// + 10;
